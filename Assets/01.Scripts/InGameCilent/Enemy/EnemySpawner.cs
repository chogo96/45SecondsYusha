...
using Photon.Pun;
using Photon.Realtime; // PhotonPlayer 클래스에 접근하기 위한 네임스페이스 추가
...
public class EnemySpawner : MonoBehaviourPunCallbacks
{
    ...
    public List<PlayerScripts> players; // 플레이어 리스트 초기화 디버그 체크용으로 public으로 변경
    ...
    private void RegisterAllPlayers()
    {
        ...
        foreach (Player player in PhotonNetwork.PlayerList)
        {
            ...
            if (playerObject != null)
            {
                ...
                if (playerScripts != null && !players.Contains(playerScripts))
                {
                    players.Add(playerScripts);
                }
            }
        }
    }
    ...
    {
        // 네트워크에서 플레이어 객체를 찾는 로직
        // 각 플레이어의 객체가 고유한 이름이나 태그로 관리되고 있다면 이를 이용해 찾을 수 있음
        return GameObject.Find(player.NickName); // 예시: 닉네임으로 찾기
    }
